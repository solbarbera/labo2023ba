...
PARAM$lgb_semilla <- 100129 # mi semilla 
...
PARAM$lgb_basicos <- list(
  ...
  verbosity = -100,
  max_depth = 5L, # -1 significa no limitar,  por ahora lo dejo fijo
  min_gain_to_split = 0.3010, # min_gain_to_split >= 0.0
  min_sum_hessian_in_leaf = 0.001, #  min_sum_hessian_in_leaf >= 0.0
  lambda_l1 = 0.8507, # lambda_l1 >= 0.0
  lambda_l2 = 0.0, # lambda_l2 >= 0.0
  max_bin = 31L, # lo debo dejar fijo, no participa de la BO
  num_iterations = 1695, # un numero muy grande, lo limita early_stopping_rounds

  bagging_fraction = 1.0, # 0.0 < bagging_fraction <= 1.0
  pos_bagging_fraction = 0.9118, # 0.0 < pos_bagging_fraction <= 1.0
  neg_bagging_fraction = 0.4114, # 0.0 < neg_bagging_fraction <= 1.0
  is_unbalance = FALSE, #
...
PARAM$bo_lgb <- makeParamSet(
  makeIntegerParam("num_leaves", lower = 410L, upper = 420L),
  makeIntegerParam("min_data_in_leaf", lower = 140L, upper = 150L)
)
