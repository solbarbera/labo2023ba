...
PARAM$lgb_basicos <- list(
  ...
  min_gain_to_split = 0.2313, # min_gain_to_split >= 0.0
  min_sum_hessian_in_leaf = 0.001, #  min_sum_hessian_in_leaf >= 0.0
  lambda_l1 = 0.0, # lambda_l1 >= 0.0
  lambda_l2 = 0.0, # lambda_l2 >= 0.0
  max_bin = 31L, # lo debo dejar fijo, no participa de la BO
  num_iterations = 3231, # un numero muy grande, lo limita early_stopping_rounds

  bagging_fraction = 1.0, # 0.0 < bagging_fraction <= 1.0
  pos_bagging_fraction = 0.9295, # 0.0 < pos_bagging_fraction <= 1.0
  neg_bagging_fraction = 0.3944, # 0.0 < neg_bagging_fraction <= 1.0
  is_unbalance = FALSE, #
...
PARAM$bo_lgb <- makeParamSet(
  makeIntegerParam("num_leaves", lower = 690L, upper = 700L),
  makeIntegerParam("min_data_in_leaf", lower = 115L, upper = 120L)
)
